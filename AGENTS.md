...
- `skills-zh/` 下所有包含简体中文的文件必须保存为 UTF-8 无 BOM；
- 在 Windows 环境下编辑中文文件时，禁止依赖默认本地编码写入，必须显式使用 UTF-8 无 BOM。
- 当某个 skill 的相关内容完成后，必须自动创建对应的 Pull Request，用于提交同步后的双语变更；
- 该 Pull Request 应包含本次变更的全部相关文件，避免只提交单侧目录或局部内容；
- 若当前不具备创建 PR 所需信息，应先补齐必要信息，再继续创建 PR。

- 针对 Greptile 审查的 PR 自动化，应使用定时轮询而不是单次等待；Greptile 未出结果时继续等待，发现阻塞问题时自动建 issue，确认无阻塞后再自动批准 PR。
